...
assert.match(card, /systemLaunchAction\(system\)/, 'card deve oferecer Abrir aplicação');
...
assert.match(card, /Inspecionar operação/, 'porta operacional deve ser explícita');
...
for (const selector of ['.system-launcher-card', '.ws-metrics', '.ws-mode-switch', '.ws-canvas-flow']) {
