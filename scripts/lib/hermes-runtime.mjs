...
function safeEnvState(options, paths = null) {
  try {
    ...
    return {
      ...
      allow_all_disabled: values.get('GATEWAY_ALLOW_ALL_USERS') !== 'true'
        && values.get('TELEGRAM_ALLOW_ALL_USERS') !== 'true',
    };
  } catch {
    return {
...
export function readHermesStatus(root, {
  ...
  return {
    ...
    provider_configured: Boolean(model),
    provider_label: model,
    project_bound: typeof configuredCwd === 'string' && resolve(configuredCwd) === resolve(root),
    skills_trusted: Array.isArray(trusted) && trusted.some((path) => {
      try { return resolve(path) === resolve(root); } catch { return false; }
    }),
...
export function bindHermesProject(root, options = {}) {
  ...
  assertSucceeded(invoke(['skills', 'trust', '--help'], invokeOptions), 'hermes-update-required');
  ...
  return { status: 'bound', skills_trusted: true };
}
